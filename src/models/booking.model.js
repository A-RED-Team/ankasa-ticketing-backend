...
const bookingModel = {
  ...
  detailBookingUser: (bookingId) => {
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT flights.departure_date, flights.departure_time, airlines.name AS airline_name, airlines.image, country1.alias AS from_contry, country2.alias AS to_contry, bookings.terminal, bookings.gate, bookings.total_ticket, flights.class, bookings.payment_status FROM flights INNER JOIN bookings ON flights.id = bookings.flight_id INNER JOIN cities AS city1 ON city1.id = flights.departure_city INNER JOIN cities AS city2 ON city2.id = flights.arrival_city INNER JOIN countries AS country1 ON country1.id = city1.country_id INNER JOIN countries AS country2 ON country2.id = city2.country_id INNER JOIN airlines ON flights.airline_id = airlines.id WHERE bookings.id='${bookingId}' AND bookings.payment_status=1 AND bookings.is_active=1`,
        (err, result) => {
          if (err) {
            reject(new Error(err.message));
          } else {
  ...
  listUserBooking: (userId, getLimit, offset) => {
    return new Promise((resolve, reject) => {
      db.query(
        `SELECT bookings.id AS booking_id, bookings.total_ticket, flights.departure_date, flights.departure_time, airlines.name AS airline_name, country1.alias AS from_contry, country2.alias AS to_contry, bookings.terminal, bookings.gate, flights.class, bookings.payment_status FROM flights INNER JOIN bookings ON flights.id = bookings.flight_id INNER JOIN cities AS city1 ON city1.id = flights.departure_city INNER JOIN cities AS city2 ON city2.id = flights.arrival_city INNER JOIN countries AS country1 ON country1.id = city1.country_id INNER JOIN countries AS country2 ON country2.id = city2.country_id INNER JOIN airlines ON flights.airline_id = airlines.id WHERE bookings.user_id='${userId}' AND bookings.is_active=1 LIMIT ${getLimit} OFFSET ${offset}`,
        (err, result) => {
          if (err) {
            reject(new Error(err.message));
          } else {
